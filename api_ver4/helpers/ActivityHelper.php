...
class ActivityHelper extends Object
{
    ...
    {
        $sql = 'SELECT COUNT(ActivityID) as count, (SELECT MAX(ActivityID) FROM memberactivity WHERE ArtistID='.$artistID.' AND PostID='.$postID.' AND MemberID='.$profileID.' AND ActivityTypeID='.$activityTypeID.') as id FROM memberactivity WHERE ArtistID='.$artistID.' AND PostID='.$postID.' AND MemberID='.$profileID.' AND ActivityTypeID='.$activityTypeID;
