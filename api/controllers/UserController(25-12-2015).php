...
class UserController extends Controller
{
   ...
   public function behaviors()
   {
      ...
      $behaviors['authenticator'] =  [
		      ...
		      'sharelist','share','flaglist','artisteditprofile',
                      'artisthomescreen','mymusic','globaliospush','globalandroidpush'],								
	      ];
      $behaviors['verbs'] =  [
   ...
   public function actionLogin()
   {
	 ...
	 if($resultCode == "200")
	 {
             //echo "<pre>";print_r(['Status'=>1,"Message"=>$resultMessage,'Result'=>$loginData]);exit;
	    echo json_encode(['Status'=>1,"Message"=>$lngmsg,'Result'=>$loginData], JSON_PRETTY_PRINT);
	 }
	 else
	 { 
	    echo json_encode(['Status'=>0,"Message"=>$lngmsg,'Result'=>$loginData], JSON_PRETTY_PRINT);
	 }   
      }
      else
      {
  ...
  public function actionCheckusername()
  {
           ...
           if(count($profileData)>0)
           {
                ...
                if($resultCode == "200")
                {
                    ...
                    if($email != "")
                    {
                        ...
                        ->setTo($email)
                        ->setSubject("Boom Video - Reset password link")
            ...
            if(count($activityData)>0)
            {
                ...
                if($userType == "3")
                {
                    if($artistdevicetype != "" && $artistdevicetoken != "")
                    {
                        ...
                        {
                            $sendpushtoartist->sendToAndroid();
                        }
                    }
                }
                if(count($gettokens) > 0)
                {
                    ...
                    if(count($iostokens) > 0)
                    {
                        try
                        {
                            $sendpushtousers->sendToIphone($iostokens,$message,$time,$postID,$totalcomments);
                        }
                        catch(Exception $e)
                        {
                            echo "cat message".$e->getMessage()." \n";
                    ...
                    {
                        $sendpushtousers->sendToAndroid($androidtokens,$message,$time,$postID,$totalcomments);
                    }
   ...
   public function actionSignup()
   {
        $arrParams 		= 	Yii::$app->request->post();
        $data 			= 	json_decode($arrParams['params']);
        $availableParams 	= 	array('ArtistID','Name','Username','Email','Birthdate','Gender','Zipcode','Mobile','Password','Image','Language');
        $compareField 		= 	array_diff_key(array_keys($arrParams),$availableParams);
        if(count($compareField) == 0)
        {
            $artistID 		= 	$data->ArtistID;
            ...
            if($resultCode == 200)
            {
                ...
                if($email != "")
                {
                    $getArtistData  =   new \backend\models\Artist();
                    $artistname     =   $getArtistData->getArtistName($artistID);
                    $link           =   '<a href="'.self::Activationpage.$activationcode.'">'.self::Activationpage.$activationcode.'</a>';
                    $getcontent     =   $getArtistData->getSignupemailtemplate($name,$artistname,$link);
                     Yii::$app->mailer->compose()
                    ->setFrom(self::Fromemail)
                    ->setTo($email)
   ...
   public function actionEditprofile()
   {
      $arrParams 		= 	Yii::$app->request->post();
      $data 			= 	json_decode($arrParams['params']);
      $availableParams 		= 	array('UserID','ProfileID','ArtistID','Name','Email','Password','Language','Birthdate','Gender','Zipcode','Mobile','Password','Image');
      $compareField 		= 	array_diff_key(array_keys($arrParams),$availableParams);
      if(count($compareField) == 0)
      {
	 $userID 		= 	$data->UserID;
   ...
   public function actionArtisteditprofile()
   {
      ...
      $availableParams 		= 	array('ArtistID','UserID','ArtistName','ProfileThumb','Email','Birthdate','Nationality',
					      ...
					      'LinkedInPageUrl','TwitterPageUrl','FacebookPageUrl','Language','DeleteMediaIDs','ArtistImages','Password');
      $compareField 		= 	array_diff_key(array_keys($arrParams),$availableParams);
      if(count($compareField) == 0)
      {
	 $artistID 		= 	$data->ArtistID;
	 ...
	 $procedure     	=	"CALL Artist_Edit_Profile(".$artistID.",".$userID.",'".$artistName."','".$profileThumb."',
				       ...
				       '".$yearsActive."','".$genre."','".addslashes($aboutMe)."','".$youTubePageUrl."','".$linkedInPageUrl."',
   ...
   public function actionPostlist()
   {
        ...
        if(count($compareField) == 0)
        {
            ...
            if($usertype == "3")
            {
                ...
                //$this->setHeader($resultCode);
				$encodedArray = array('Status'=>$status,"Message"=>utf8_encode($lngmsg),"RecordCount"=>$recordcnt,"Result"=>$rssaray);
                echo json_encode($encodedArray,JSON_PRETTY_PRINT); //JSON_PRETTY_PRINT
            }
            else
            {
                echo json_encode(['Status'=>$status,"Message"=>utf8_encode($lngmsg),"RecordCount"=>$recordcnt,"Result"=>$postData], JSON_PRETTY_PRINT);
            }
        }
        else
        {
        ...
        if(count($compareField) == 0)
        {
           ...
           if(count($postData)>0)
           {
              ...
              if($postType == "2" || $postType == "3")//Images
              {
                 ...
                 {
                      $gallComm		=	"CALL Post_Add_Media(".$artistID.",".$postID.",".$postType.",'".$value."','".$videothumb."','".self::BoomPath."','".self::S3BucketPostVideos."')";
   ...
   public function actionGlobaliospush()
   {
        ...
        if(count($iostokens) > 0)
        {
            ...
            {
                $sendpush->sendToIphone($uniquebatch[$i], $batchMessage[$i], $currenttime);
            }
   ...
   public function actionGlobalandroidpush()
   {
        ...
        if(count($androidtokens) > 0)
        {
            ...
            for($i=0;$i<count($uniquebatch);$i++) 
            {
                $sendpush->sendToAndroid($uniquebatch[$i], $batchMessage[$i], $currenttime);
            }
