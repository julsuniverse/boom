   ...
   public function behaviors()
   {
      ...
      $behaviors['authenticator'] =  [
		      ...
		      'sharelist','share','flaglist','artisteditprofile',
                      'artisthomescreen','mymusic','globaliospush','globalandroidpush','latestcommentlist','addfeed'],								
	      ];
      $behaviors['verbs'] =  [
  ...
  return date(preg_replace('`(?<!\\\\)u`', $milliseconds, $format), $timestamp);
   ...
   public function actionLogin()
   {
	 ...
	 //$resultMessage = 	$loginData[0]['Message'];
        ...
        if($resultCode == "200")
        {
           echo json_encode(['Status'=>1,"Message"=>$lngmsg,'Result'=>$loginData,'RecordCount'=>$recordcnt,'MemberProfile'=>$userData,'ArtistProfile'=>$artistData,'PostList'=>$postData,'SimilarApp'=>$similarApp], JSON_PRETTY_PRINT);
        }
        else
        { 
           echo json_encode(['Status'=>0,"Message"=>$lngmsg,'Result'=>$loginData,'RecordCount'=>$recordcnt,'MemberProfile'=>$userData,'ArtistProfile'=>$artistData,'PostList'=>$postData,'SimilarApp'=>$similarApp], JSON_PRETTY_PRINT);
        }   
      }
      else
      {
  ...
  public function actionCheckusername()
  {
   ...
   public function actionStickers()
   {
	 ...
	 $deviceType 		= 	$data->DeviceType;
         ...
         $artistID 		= 	$data->ArtistID;
	 $connection 		= 	Yii::$app->db;
	 $procedure		=	"CALL Member_Sticker_List(".$memberID.",".$artistID.",".$stickerType.",".$deviceType.",'".self::S3BucketAbsolutePath."/','".self::S3BucketStickers."','".self::S3BucketStickersSmall."','".self::S3BucketStickersMedium."')";
	 $command 		= 	$connection->createCommand($procedure);     
