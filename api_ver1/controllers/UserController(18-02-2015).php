...
class UserController extends Controller
{
  ...
  return date(preg_replace('`(?<!\\\\)u`', $milliseconds, $format), $timestamp);
   ...
   public function actionLogin()
   {
	 ...
	 //$resultMessage = 	$loginData[0]['Message'];
        ...
        if($resultCode == "200")
        {
           echo json_encode(['Status'=>1,"Message"=>$lngmsg,'Result'=>$loginData,'MemberProfile'=>$user_profileData[0],'ArtistProfile'=>$artist_profileData[0],'PostList'=>$postData,'SimilarApp'=>$similarApp], JSON_PRETTY_PRINT);
        }
        else
        { 
           echo json_encode(['Status'=>0,"Message"=>$lngmsg,'Result'=>$loginData,'MemberProfile'=>$user_profileData[0],'ArtistProfile'=>$artist_profileData[0],'PostList'=>$postData,'SimilarApp'=>$similarApp], JSON_PRETTY_PRINT);
        }   
      }
      else
      {
  ...
  public function actionCheckusername()
  {
   ...
   public function actionSignup()
   {
        $arrParams 		= 	Yii::$app->request->post();
        $data 			= 	json_decode($arrParams['params']);
        $availableParams 	= 	array('ArtistID','Name','Username','Email','Mobile','Password','Image','Language');
        $compareField 		= 	array_diff_key(array_keys($arrParams),$availableParams);
        if(count($compareField) == 0)
        {
            $artistID 		= 	$data->ArtistID;
            ...
            if($resultCode == 200)
            {
                ...
                if($email != "")
                {
                    ...
                    $getcontent     =   $getArtistData->getSignupemailtemplate($name,$artistname,$link,$language);
                     Yii::$app->mailer->compose()
                    ->setFrom(self::Fromemail)
                    ->setTo($email)
