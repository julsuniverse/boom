...
//require_once '../../api_ver1/models/Commentnotforartist.php';
/**
...
class UserController extends Controller
{
   ...
   private $arrSkipAction = ['create', 'login','addfeed'];
   ...
   public function behaviors()
   {
      ...
      $behaviors['authenticator'] =  [
		      ...
		      'sharelist','share','flaglist','artisteditprofile',
                      'artisthomescreen','mymusic','mymusictv','globaliospush','globalandroidpush','latestcommentlist','addfeed','testpush'],								
	      ];
      $behaviors['verbs'] =  [
  ...
  return date(preg_replace('`(?<!\\\\)u`', $milliseconds, $format), $timestamp);
   ...
   public function actionLogin()
   {
	 ...
	 //$resultMessage = 	$loginData[0]['Message'];
        ...
        if($usertype == "3")
        {
            $postData_proc               =	"CALL Post_List_Ver1(".$artistID.",".$userID.",".$profileID.",0,'".self::S3BucketPath."','".self::Postvideosthumb."','".self::Postblurthumbvideos."',".self::Limit.",1,@o_RecCount)";
        ...
        /************************ Sticker List ***************************/
        $stickersData = array();
        $procedure		=	"CALL Sticker_List(".$artistID.",".$data->DeviceType.",'".self::S3BucketAbsolutePath."/','".self::BoomFolder.$artistID.self::S3BucketStickers."','".self::BoomFolder.$artistID.self::S3BucketStickersSmall."','".self::BoomFolder.$artistID.self::S3BucketStickersMedium."')";
        ...
        if($resultCode == "200")
        {
           echo json_encode(['Status'=>1,"Message"=>$lngmsg,'Result'=>$loginData,'RecordCount'=>$recordcnt,'MemberProfile'=>$userData,'ArtistProfile'=>$artistData,'PostList'=>$postData,'SimilarApp'=>$similarApp,'stickersData'=>$stickersData,'BucketName'=>"boomvideoqa-in/",'CognitoPoolId'=>"ap-northeast-1:242370fe-fca5-47e0-8146-de32d235a20c"], JSON_PRETTY_PRINT);
        }
        else
        { 
           echo json_encode(['Status'=>0,"Message"=>$lngmsg,'Result'=>$loginData,'RecordCount'=>$recordcnt,'MemberProfile'=>$userData,'ArtistProfile'=>$artistData,'PostList'=>$postData,'SimilarApp'=>$similarApp,'stickersData'=>$stickersData,'BucketName'=>"boomvideoqa-in/",'CognitoPoolId'=>"ap-northeast-1:242370fe-fca5-47e0-8146-de32d235a20c"], JSON_PRETTY_PRINT);
        }   
      }
      else
      {
  ...
  public function actionCheckusername()
  {
   ...
   public function actionLikecomment()
   {
    ...
    public function actionTestpush() {
        ...
        $sendpushtoartist->deviceToken  =    '9dc7c3e58639e1e10f79f8fdaf9753ff3424414a4d76a06d3ca97ffc1027aa36';
        $sendpushtoartist->message      =    'dfdfdf';
        $sendpushtoartist->time         =    date("Y-m-d H:i:s");
        $sendpushtoartist->postid       =    '1';
        $sendpushtoartist->NotificationType       =    "comment";
        $sendpushtoartist->sendToIphone(4);
        echo 'sdsd'; die;
    }
        ...
        if(count($compareField) == 0)
        {
            ...
            if($resultCode == 200)
            {
                ...
                if($email != "")
                {
                    $getArtistData  =   new \backend\models\Artist();
                    $artistname     =   $getArtistData->getArtistName($artistID);
                    $link           =   self::Activationpage.$activationcode.'&appname='.$subject;
   ...
   public function actionArtisteditprofile()
   {
	 ...
	 $password		=	$data->Password;
         $language		=	$data->Language;
	 $connection 		= 	Yii::$app->db;
	 $procedure     	=	"CALL Artist_Edit_Profile_Ver1(".$artistID.",".$userID.",'".$artistName."','".$profileThumb."',
   ...
   public function actionAddfeed() {
        ...
        $xmlmediathumb = '';
        $xmlcontenturl = '';
        if(isset($items[0]['title']) && $items[0]['title']!='')
            $xmltitle          =   $items[0]['title'];
        if(isset($items[0]['description']) && $items[0]['description']!='')
            $xmldesc           =   $items[0]['description'];
        if(isset($items[0]['media:title']) && $items[0]['media:title']!='')
            $xmlmediatitle     =   $items[0]['media:title'];
        if(isset($items[0]['media:thumbnail']['url']) && $items[0]['media:thumbnail']['url']!='')
            $xmlmediathumb          =   $items[0]['media:thumbnail']['url'];
        if(isset($items[0]['media:content']['url']) && $items[0]['media:content']['url']!='')
            $xmlcontenturl          =   $items[0]['media:content']['url'];   
   ...
   public function actionPostlist()
   {
       $logString  = "";
        ...
        if(count($compareField) == 0)
        {
            ...
            $language               = 	$data->Language;
            $connection             = 	Yii::$app->db;
            $procedure              =	"CALL Post_List_Ver1(".$artistID.",".$UserID.",".$profileID.",".$isExclusive.",'".self::S3BucketPath."','".self::Postvideosthumb."','".self::Postblurthumbvideos."',".self::Limit.",".$pageindex.",@o_RecCount)";
            //echo $procedure; die;
            $command                =	$connection->createCommand($procedure);     
		...
		$encodedArray = array('Status'=>$status,"Message"=>$lngmsg,"RecordCount"=>$recordcnt,"Result"=>$postData);
                echo json_encode($encodedArray,JSON_PRETTY_PRINT); 
            }
            else
            {
   ...
   public function actionStickers()
   {
      $arrParams 		= 	Yii::$app->request->post();
      $data 			= 	json_decode($arrParams['params']);
      $availableParams 		= 	array('ProfileID','StickerType','DeviceType','Language','ArtistID');
      $compareField 		= 	array_diff_key(array_keys($arrParams),$availableParams);
      if(count($compareField) == 0)
      {
	 ...
	 $deviceType 		= 	$data->DeviceType;
         ...
         $artistID 		= 	$data->ArtistID;
	 $connection 		= 	Yii::$app->db;
	 $procedure		=	"CALL Member_Sticker_List(".$memberID.",".$artistID.",".$stickerType.",".$deviceType.",'".self::S3BucketAbsolutePath."/','".self::BoomFolder.$artistID.self::S3BucketStickers."','".self::BoomFolder.$artistID.self::S3BucketStickersSmall."','".self::BoomFolder.$artistID.self::S3BucketStickersMedium."')";
	 $command 		= 	$connection->createCommand($procedure);     
	 $stickersData 		= 	$command->queryAll();
   ...
   public function actionAddpost()
   {
      ...
      $logString.="\n Params : ".$arrParams['params'].'\n';	
        $data 			= 	json_decode($arrParams['params']);
        $availableParams 		= 	array('PostTitle','PostType','Language','Description','ArtistID','IsExclusive','IsShared','Media','VideoThumbImage');
        $compareField 		= 	array_diff_key(array_keys($arrParams),$availableParams);
        $s3 = new \S3(AWSACCESSKEY, AWSSECRETKEY); 
        if(count($compareField) == 0)
        {
           ...
           $connection       	= 	Yii::$app->db;
           $proc			=	"CALL Post_Add_Ver1('".addslashes($postTitle)."','".$postType."','".addslashes($description)."','".$artistID."','".$isExclusive."','".$isShared."','".$price."','".$productID."')";
           $logString.="\n Post Add : ".$proc.'\n';	
              ...
              if($postType == "2" || $postType == "3")//Images
              {
                 ...
                 foreach ($media as $key=>$value)
                 {
                     ...
                     else
                     {
                      ...
                      $gallComm		=	"CALL Post_Add_Media_Ver1(".$artistID.",".$postID.",".$postType.",'".$value."','".$thumbvalue."','".$videothumb."','".self::BoomPath."','".self::S3BucketPostVideos."')";
           ...
           /****************************** Push for Video and Images *************************/
                /*if($postType == "1" || $postType == "2" || $postType == "3") {
                    ...
                    foreach($iosToken as $key => $value) {
                        ...
                        {  
                            $ios[] = trim($value['DeviceToken']);
                        }    
                    }
                    $artistobj          =   new \backend\models\Artist();
                    $sendpushtoartist       =   new \api\models\Commentnotforartist();
                    $getmsgforartist        =   $artistobj->getBulkNotificationMessageForPost($memberID,$artistID,$postType,$postTitle,$description);
                    $sendpushtoartist->deviceToken  =    $ios;
                    $sendpushtoartist->message      =    $getmsgforartist;
                    $sendpushtoartist->time         =    date("Y-m-d H:i:s");
                    $sendpushtoartist->postid       =    "";
                    $sendpushtoartist->NotificationType       =    "addPost";
                    $sendpushtoartist->sendToBulkIphone($artistID,'user');   
                    ...
                    $sendpushtoartist->deviceToken  =    $android;
                    $sendpushtoartist->message      =    $getmsgforartist;
                    $sendpushtoartist->time         =    date("Y-m-d H:i:s");
                    $sendpushtoartist->postid       =    "";
                    $sendpushtoartist->NotificationType       =    "addPost";
                    $sendpushtoartist->sendToBulkAndroid($artistID,'user');   
                }*/
            /*********************************************************************************/ 
   ...
   public function actionPurchasepost()
   {
	 ...
	 $connection       	= 	Yii::$app->db;
	 $procedure 		=	"CALL Post_Purchase(".$artistID.",".$memberID.",".$postID.",".$deviceType.")";
	 $command 		= 	$connection->createCommand($procedure);     
	 $postData 		= 	$command->queryAll();
	 if(count($postData)>0)
	 {
   ...
   public function actionMymusictv()
   {
        ...
        if(count($compareField) == 0)
        {
            ...
            $lngmsg = \Yii::t('api',$resultMessage);
            $this->setHeader(400);
            echo json_encode(["Status"=>$status,"Message"=>$lngmsg,"Result"=>$trackData], JSON_PRETTY_PRINT);
        }
        else
        {
            $resultMessage 	= 	_getStatusCodeMessageMymusic(502);
            ...
            $lngmsg = \Yii::t('api',$resultMessage);
            $this->setHeader(400);
            echo json_encode(['Status'=>0,"Message"=>$lngmsg,"Result"=>array()],JSON_PRETTY_PRINT);
        }
   }
