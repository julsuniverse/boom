...
require_once '../../backend/models/Member.php';
require_once '../../backend/models/Nativeproducts.php';
require_once '../../backend/models/Memberpost.php';
require_once '../../backend/models/Feedarticles.php';
require_once '../../backend/models/PostPages.php';
    ...
    public function actionLogin() {
        ...
        try
        {
            ...
            if (count($compareField) == 0)
            {
                ...
                if($resultCode=="404"){
                 //$usermigrate = User::find()->where(['and','Username ='.AES_ENCRYPT($username,PASSWORD("boom123")),'Is_migrate=1'])->all();
                 //$usermigrate = User::find()->where(['and','Username ='.$username,'Is_migrate=1'])->all();
                 ...
                 //    echo $result;
                 // }
                 if($result) $loginData[0]['Is_migrate']=1;
                }
				...
				if($artistProfileID!=0){
                    ...
                    // {
                        ...
                        if($ComID==0) {
                            ...
                            $postData_proc = "CALL Post_List(0," .  $artistID . "," . $userID . "," . $profileID . "," . $ComID . "," . $isExclusive . ",'" . self::S3BucketPath . "','" . self::Postvideosthumb . "','" . self::Postblurthumbvideos . "','','',@o_RecCount)";//Daniele
    ...
    public
        ...
        try
        {
            ...
            if (count($compareField) == 0 && isset($data->Email))
            {
                ...
                $connection = Yii::$app->db;
                /************** Registr Member ****************/
                $procedure="CALL Member_Registration('" . $name . "','" . $image . "','" . $email . "','" . $birthDate . "','" . $zipCode . "','" . $mobile . "','" . $gender . "','" . $artistID . "','" . $username . "','" . $devicetype . "','" . self::EncryptKey . "','" . $password . "', " . $ComID . ")";
                $command = $connection->createCommand($procedure);
                $logString.="\n ".$procedure;    
...
public function actionEditprofile() {
        ...
        try
        {
            ...
            if (count($compareField) == 0)
            {
                ...
                $connection = Yii::$app->db;
                $procedure = "CALL Member_Edit_Profile_new('". $username . "','" . $name . "','" . $image . "','" . $email . "','" . $password . "','" . $birthDate . "','" . $zipCode . "','" . $mobile . "','" . $gender . "','" . $artistID . "','" . $profileID . "','" . $userID . "'," . $showNotification . ",'" . self::EncryptKey . "')";
    ...
    public
        ...
        try
        {
            ...
            if (count($compareField) == 0)
            {
                ...
                // //added by Kate--deeplinking
                // $procedure = "CALL Post_List(0, " . $artistID . "," . $UserID . "," . $profileID . "," . $isExclusive . ",'" . self::S3BucketPath . "','" . self::Postvideosthumb . "','" . self::Postblurthumbvideos . "','','',@o_RecCount)";//Daniele
                
                $procedure = "CALL Post_List(0, " . $artistID . "," . $UserID . "," . $profileID . "," . $ComID . "," . $isExclusive . ",'" . self::S3BucketPath . "','" . self::Postvideosthumb . "','" . self::Postblurthumbvideos . "','','',@o_RecCount)";//Daniele
                ...
                if (count($postData) > 0)
                {
                    ...
                    if($postType == "3"&&$viedolink_u!=""){
                        if (isset($data->mobilevideoThumbImage) && $data->mobilevideoThumbImage != ""){
                            ...
                            $connection2 = \Yii::$app->db;
                            $r2=$connection2->createCommand()->update('post', array("MobileStreamUrl"=>$viedolink_u),'PostID = '.$postID)->execute();
                            if ($videothumb != "")
                            {
                            
                                //Added by Dan: if QAType = image, save url for Thumbimage too
                                $thumburl = self::S3BucketAbsolutePath.'/boom-'.$artistID.POST_THUMBIMAGE_VIDEOS.$videothumb;
                                $connection = \Yii::$app->db;
                                $connection->createCommand()->update('post', array("VideoThumbImage"=>$thumburl),'PostID = '.$postID)->execute();
                            
                                //echo self::S3BucketAbsolutePath.'/'.self::BoomFolder.$artistID.self::Postvideosthumb.$videothumb; 
	...
	public function actionTestpushdan(){
		...
		if($deviceType == 1){
			...
			$res = $sendpushtoartist->sendToIphone($artistID, 'user');
		}
		echo $res;
	}
